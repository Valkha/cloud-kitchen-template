...
import { ChevronRight, Star, Loader2, AlertTriangle } from "lucide-react";
...
export default function BrandsSection() {
  ...
  useEffect(() => {
    async function fetchBrands() {
      try {
        ...
        // ✅ Correction ESLint : On évite 'any' en vérifiant l'instance de Error
  ...
  if (error) {
    return (
      <div className="py-40 flex flex-col items-center justify-center gap-4 text-red-500 text-center px-6">
        <AlertTriangle size={40} />
        <p className="text-[10px] font-black uppercase tracking-widest max-w-xs">
          Liaison perdue : {error}
        </p>
        <button 
          onClick={() => window.location.reload()}
          className="mt-4 text-[8px] font-black uppercase tracking-tighter border border-red-500/30 px-4 py-2 rounded-full hover:bg-red-500 hover:text-white transition-all"
        >
          Tenter un nouveau scan
        </button>
      </div>
    );
  }
              ...
              >
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-white pt-4 border-t border-white/5">
                  ...
                  <div className="absolute bottom-4 right-8 text-[8px] font-black uppercase tracking-[0.3em] text-neutral-800">
                    Station_{brand.id.toString().slice(0, 4).toUpperCase()}
                  </div>
                </m.div>
              </TransitionLink>
            </Reveal>
        ...
        {!isLoading && brands.length === 0 && (
          <div className="text-center py-20 border border-dashed border-white/10 rounded-[2.5rem]">
            <p className="text-neutral-500 text-[10px] font-black uppercase tracking-widest italic">
              Aucun signal détecté. Vérifiez la base de données.
            </p>
          </div>
        )}
      </div>
