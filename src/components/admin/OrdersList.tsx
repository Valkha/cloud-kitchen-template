...
export default function OrdersList() {
  ...
  useEffect(() => {
    ...
    const savedSoundPref = localStorage.getItem("planetfood_admin_sound");
  ...
  const toggleSound = () => {
    ...
    localStorage.setItem("planetfood_admin_sound", String(nextState));
  ...
  const getStatusStyle = (status: string) => {
    ...
    if (s === "annulée" || s === "cancelled") return { bg: "bg-red-900/20", text: "text-red-500", border: "border-red-900/30", icon: <XCircle size={10} />, next: null, btnLabel: "", btnIcon: null };
          ...
          {orders.map((order) => {
            ...
            return (
              <motion.div 
                ...
                className={`bg-neutral-900 border border-neutral-800 rounded-2xl p-5 flex flex-wrap md:flex-nowrap items-center justify-between gap-6 hover:border-neutral-700 transition shadow-xl ${(order.status === 'delivered' || order.status === 'cancelled') ? 'opacity-40 grayscale' : ''}`}
              >
                {/* NUMÉRO ET CLIENT */}
